...
function v4()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("phongbeo") then
			makefolder("phongbeo")
		end
		if not isfolder("phongbeo/Blox Fruits/") then
			makefolder("phongbeo/Blox Fruits/")
		end
		if not isfile("phongbeo/Blox Fruits/phongbeo.json") then
			writefile("phongbeo/Blox Fruits/phongbeo.json", game:GetService("HttpService"):JSONEncode(getgenv().Settings))
		else
			local sk = game:GetService("HttpService"):JSONDecode(readfile("phongbeo/Blox Fruits/phongbeo.json"))
...
function v5()
	if readfile and writefile and isfile and isfolder then
		if not isfile("phongbeo/Blox Fruits/phongbeo.json") then
			...
			local hsiw = game:GetService("HttpService"):JSONDecode(readfile("phongbeo/Blox Fruits/phongbeo.json"))
			local input = {}
			for quiqui, ququ in pairs(getgenv().Settings) do
				input[quiqui] = ququ
			end
			writefile("phongbeo/Blox Fruits/phongbeo.json", game:GetService("HttpService"):JSONEncode(input))
		end
	else
		return
	end
